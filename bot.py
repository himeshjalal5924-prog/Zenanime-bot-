...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 Welcome! Use links or wait for files.")
...
async def list_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async for f in cursor:
        ...
        keyboard.append([
            InlineKeyboardButton(name, callback_data=f"get_{key}")
        ])
...
async def file_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("get_"):
        ...
        if file_data:
            msg = await query.message.reply_document(file_data["file_id"])
